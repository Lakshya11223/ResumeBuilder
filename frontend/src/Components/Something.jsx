...
import { CirclePlus, FileText, Trash2, SquarePen, Award } from "lucide-react" 
...
function Something() {
  ...
  const { loading, getall, resumes, Delete, getresume } = useResumeStore();
    ...
    <>
      <div className='relative flex pt-25 sm:pt-20 min-h-screen w-full bg-gradient-to-r from-slate-50 via-amber-50 to-blue-50 overflow-y-scroll'>
        ...
        <div className='flex flex-wrap '>
          <div className='m-5 sm:m-10 flex flex-col justify-center items-center h-60 sm:h-70 w-65 sm:w-70 gap-2 border-2 border-dashed rounded-xl shadow-lg bg-gradient-to-r from-slate-50 to-blue-50 hover:-translate-y-1 transition-transform duration-500'>
          ...
          {resumes.map((data, idx) => (
            ...
            >
              <button onClick={() => getresumehandler(data._id)} className='hover:opacity-80 cursor-pointer'>
                <div className='bg-gradient-to-r from-blue-600 to-purple-600 shadow-lg p-2 rounded-xl mb-5 cursor-pointer'>
              ...
              <div className='max-w-60 flex gap-4 mt-1'>
                ...
                {/* Score button */}
                <button className='p-1' onClick={() => navigate(`/score/${data._id}`)}>
                  <Award className='size-7 cursor-pointer text-emerald-500 hover:opacity-70'/>
                </button>

                <button onClick={() => handledelete(data._id)} >
                  <Trash2 className="size-7 cursor-pointer text-red-500 hover:opacity-70 " />
                </button>
              </div>
            </div>
          ))}
